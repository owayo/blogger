...
String.prototype.normarize = function(){
  ...
  // 濁点
  d = {
    0x30AB:1,0x30ad:1,0x30af:1,0x30b1:1,0x30b3:1,0x30b5:1,0x30b7:1,0x30b9:1,0x30bb:1,0x30bd:1,0x30bf:1,0x30c1:1,0x30c4:1,0x30c6:1,0x30c8:1,0x30cf:1,0x30d2:1,0x30d5:1,0x30d8:1,0x30db:1,
    0x30a6:78
  };
  // 半濁点
  hd = {
    0x30cf:2,0x30d2:2,0x30d5:2,0x30d8:2,0x30db:2
  };
...
Number.prototype.zeroPadding = function(n){
  var s=this.toString(),p="0".times(n);
  if(n<=s.length){
    return s;
  }else{
    return (p+s).slice(-n);
...
String.prototype.to_hexcolor = function(){
  var s=this,p="0".times(6);
  return (p+s).slice(-6);
};
String.prototype.zeroPadding = function(n){
  var s=this,p="0".times(n);
  if(n<=s.length){
    return s;
  }else{
    return (p+s).slice(-n);
...
function StorageSave(key,obj,k){
  if(localStorage && localStorage.setItem){
    localStorage.setItem( key+"_key",k.toString() );
...
function validator(){
  ...
  function floatValidate(e){
    ...
    if(!e.ctrlKey && e.keyCode != 8 && e.keyCode != 46){
      // 数字、ドットは許可
      if(48 <= e.keyCode && e.keyCode <= 57 || e.keyCode == 190){
      }else{
        cancel(e);
      }
    }
